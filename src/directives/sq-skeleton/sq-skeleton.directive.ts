...
export class SqSkeletonDirective implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['skeleton'] || changes['skeletonRepeat'] || changes['skeletonWidth'] || changes['skeletonHeight']) {
      this.updateView();
    }
  }
  ...
  private createSkeletons(): void {
    ...
    for (let i = 0; i < this.skeletonRepeat; i++) {
      ...
      if (this.skeletonDelay && i > 0) {
        this.renderer.setStyle(skeletonElement, 'animation-delay', `${i * this.skeletonDelay}ms`);
      }
